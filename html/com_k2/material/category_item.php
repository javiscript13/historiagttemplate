...
$articleImage = JURI::root().'images/logo-historia-gt.jpg';
$changed = false;
//try to get the image from the text
$firstImageFound = preg_match('/(<img)([^>])*(src=["\']([^"\']+)["\'])([^>])*/i', $this->item->introtext, $matches);
if($firstImageFound) {
    $articleImage = JURI::root().$matches[4];
    $changed = true;
}
//try to get the image from any attachment
if (count($this->item->attachments)) {
...
if($this->item->params->get('catItemImage') && !empty($this->item->image) && ($changed == false)) {
    $articleImage = $this->item->image;
}
?>
...
<div class="catItemView group<?php echo ucfirst($this->item->itemGroup); ?><?php echo ($this->item->featured) ? ' catItemIsFeatured' : ''; ?><?php if($this->item->params->get('pageclass_sfx')) echo ' '.$this->item->params->get('pageclass_sfx'); ?>">
	...
	<div class="catItemHeader jumbotron" id="jumbo-<?php echo $this->item->id; ?>">
	    <?php
		    $style = '#jumbo-'.$this->item->id.' {'
			    . 'background-image:linear-gradient(rgba(0, 0, 0, 0.25),rgba(0, 0, 0, 0.25)),' 
			    . 'url('.$articleImage.');'
			    . '}'
			    . '#jumbo-'.$this->item->id.':hover {'
